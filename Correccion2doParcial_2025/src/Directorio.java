...
public class Directorio {
    ...
    private boolean tokenActivo = false;private static class NodoInfo {
        final String nombre;
        final String ip;
        final int puerto;
    ...
    private synchronized void registrarNodo(String nombre, String ip, int puerto) {
        if (tokenActivo) {
            for (NodoInfo nodo : nodos) {
                LogHandler.getInstance().log(nodo.ip, nodo.puerto, nodo.nombre + " TOKEN_DELETE");
            }
            tokenActivo = false;
        }
    ...
    private synchronized void desregistrarNodo(String nombre) {
        ...
        if (nodoEliminado != null) {
            if (tokenActivo) {
                LogHandler.getInstance().log(nodoEliminado.ip, nodoEliminado.puerto, nodoEliminado.nombre + " TOKEN_DELETE");
                tokenActivo = false;
            }
            logArea.append("Nodo desregistrado: " + nodoEliminado.nombre + "\n");
        }
        nodos.removeIf(n -> n.nombre.equals(nombre));
        nodosListos.remove(nombre);
        nodoAgregado = true; // Permitir generación de nuevo token
        actualizarGUI();
        reconfigurarAnillo();
    }
    ...
    private void reconfigurarAnillo() {
        ...
        for (int i = 0; i < nodos.size(); i++) {
            ...
            if (i == 0 && nodoAgregado && !tokenActivo) {
                mensaje += " *";
                tokenActivo = true;
                logArea.append("Generando nuevo token para " + actual.nombre + "\n");
            }
            logArea.append("Enviando a " + actual.nombre + ": " + mensaje + "\n");
            notificarSiguiente(actual, mensaje);
        }
        nodoAgregado = false;
    }

    private void notificarSiguiente(NodoInfo nodo, String mensaje) {
//        try {
//            Thread.sleep(100);
//        } catch (InterruptedException ignored) {}
        for (int i = 0; i < 5; i++) {
            try (Socket s = new Socket(nodo.ip, nodo.puerto);
                ...
                logArea.append("Respuesta de " + nodo.nombre + ": " + response + "\n");
                return;
            } catch (IOException e) {
